...
def obtener_fecha_ultima_release(repo):
    ...
    try:
        ...
        print(f"[ERROR] al obtener releases de {repo}: {e}")
    return None
...
def dibujar_encabezado():
    ...
    texto_version = fuente.render("Lanzador v1.0", True, COLOR_TEXTO)
...
def dibujar_panel():
    ...
    if modo_config:
        ...
        texto = fuente_pestañas.render("¿Cada cuánto quieres revisar actualizaciones? (en minutos):", True, COLOR_TEXTO)
    ...
    if tabs[tab_activa] == "General":
        ...
        texto = fuente_pestañas.render(f"Instalación activa: {os.path.basename(ruta_instalacion_seleccionada[juegos[juego_activo]] or 'Ninguna')}", True, COLOR_TEXTO)
        screen.blit(texto, (x + 20, y + 20))
        ...
        if subpestaña_versiones_activa == 0:
            ...
            adv = fuente_pestañas.render("Selecciona una instalación local o descargada desde GitHub", True, (255, 255, 255))
            ...
            adv = fuente_pestañas.render("Selecciona una versión para descargar", True, (255, 255, 255))
            screen.blit(adv, (x + 20, y + 60))
        ...
        if not elementos:
            texto = fuente.render("No se encontraron versiones.", True, COLOR_TEXTO)
        ...
        seleccion = ruta_instalacion_seleccionada[juegos[juego_activo]]
        if not seleccion:
            texto = fuente.render("No hay instalación activa.", True, COLOR_TEXTO)
            screen.blit(texto, (x + 20, y + 20))
            return None

        ruta_mods = os.path.join(seleccion, "mods")
        if not os.path.isdir(ruta_mods):
            texto = fuente.render("No se encontró la carpeta 'mods'.", True, COLOR_TEXTO)
            screen.blit(texto, (x + 20, y + 20))
            return None

        archivos = os.listdir(ruta_mods)
        if not archivos:
            texto = fuente.render("La carpeta 'mods' está vacía.", True, COLOR_TEXTO)
            screen.blit(texto, (x + 20, y + 20))
            return None

        adv = fuente_pestañas.render("¡ADVERTENCIA!", True, COLOR_TAB_ACTIVA)
        screen.blit(adv, (x + 250, y + 20))
        adv1 = fuente_pestañas.render("Los mods son proporcionados por terceros y no son respaldados por el equipo oficial.", True, COLOR_TAB_ACTIVA)
...
def dibujar_ventana_confirmacion(texto_linea1, texto_linea2):
    ...
    texto_si = fuente_bold.render("Sí", True, (255, 255, 255))
    ...
    texto_no = fuente_bold.render("No", True, (255, 255, 255))
...
hilo_precarga = threading.Thread(target=precargar_releases, daemon=True)
hilo_precarga.start()
revisar_nuevas_releases()
...
while True:
    ...
    if modo_config and resultado_panel:
        juego_activo = -1
        boton_mas_rect, boton_menos_rect = resultado_panel
    else:
        boton_mas_rect = boton_menos_rect = None

    if mostrar_confirmacion_eliminar:
        boton_si_rect, boton_no_rect = dibujar_ventana_confirmacion(
            "¿Deseas eliminar la instalación seleccionada?",
